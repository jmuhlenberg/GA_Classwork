...
const makeMiddleEarth = () => {
  ...
  const $middleEarth = $('<div id=middle-earth>')
  ...
  for(i=0; i<lands.length; i++){
    ...
    const $landName = $('<h1>').text(lands[i])
...
const makeHobbits = () => {
  ...
  // 2. make each hobbit an li element and append it to the 'ul' you just created
    ...
    for(i=0; i<hobbits.length; i++){
      ...
      const $hobbitLI = $('<li>').attr('class', 'hobbit').text(hobbits[i])
      $hobbitsList.append($hobbitLI)
    }
...
const keepItSecretKeepItSafe = () => {
  ...
  // 2. add the ring as a child of Frodo
    ...
    const $frodoHideRing = $('.hobbit').eq(0)
    //console.log($('.hobbit').eq(0));
    $frodoHideRing.append($theRing)
...
const makeBaddies = () => {
  ...
  for(i=0; i<baddies.length; i++){
    const $baddieLI = $('<li>').attr('class', 'baddy').text(baddies[i])
    $baddiesList.append($baddieLI)
  }
...
const makeBuddies = () => {
  ...
  $middleEarth.append($buddies)

  // 2. display an unordered list of buddies in the aside
  const $buddiesList = $('<ul>')
  for(i=0; i<buddies.length; i++){
  // 3. give each of the buddies a class of "buddy"
    const $buddiesLI = $('<li>').attr('class', 'buddy').text(buddies[i])
    $buddiesList.append($buddiesLI)
  }
  // 5. don't forget to append them to the aside
  $('aside').append($buddiesList)

  // 4. append the ul to the shire
  $('#The-Shire').append($buddies)
